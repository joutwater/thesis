...
function RadarChart(id, data, options) {
	...
	if('undefined' !== typeof options){
	  ...
	  for(var i=0; i<data[0].length; i++){
	    extent_Value.push(d3.extent(data.map(d => d[i].value)));
	  };
	...
	var rScale2 = extent_Value.map(el => d3.scale.linear()
					.domain(el)
					.range([100, radius]))
	
					
					// (function(d,i) { return min_Value_Normalize[i](d.value); })
	...
	axis.append("line")
		.attr("x1", 0)
		.attr("y1", 0)
		.attr("x2", function(d, i){ return rScale(maxValue*1.0) * Math.cos(angleSlice*i - Math.PI/2.13); })
		.attr("y2", function(d, i){ return rScale(maxValue*1.0) * Math.sin(angleSlice*i - Math.PI/2.13); })
		.attr("class", "line")
